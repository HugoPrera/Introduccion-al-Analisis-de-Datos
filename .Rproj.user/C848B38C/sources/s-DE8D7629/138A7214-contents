...
setwd("/Users/maldanap/Projects/Rstudio/Introduction-to-R")

## Importar desde excel 
install.packages(readxl); 
library(readxl)
dataset <- read_excel("/Users/maldanap/Projects/Rstudio/Ventas.xlsx")
View(dataset)
...
## Representar en un diagrama de dispersión  number of orders vs sales
plot(sales$num_of_orders,sales$sales_total,main = "Number of Orders vs Sales")

## Outlier : es una observación anormal y extrema en una muestra estadística o serie temporal 
## de datos que puede afectar potencialmente a la estimación de los parámetros del mismo.

## Realizar analisis estadistico (Regresión lineal)
## lm se utiliza para ajustar modelos lineales, incluidos los multivariantes. 
## Se puede utilizar para realizar regresiones, análisis de varianza de estrato único y 
...
## P-Value se define como la probabilidad de que un valor estadístico calculado sea posible dada una 
## hipótesis nula cierta. En términos simples, el valor p ayuda a diferenciar resultados 
## que son producto del azar del muestreo, de resultados que son estadísticamente significativos.
...
## Describir la información grabada
View(sales)

## Leer la tabla con header y separado por comas
...
## Leer la tabla para generar en otro formato de archivo
sales_table_txt <- read.table("yearly_sales.csv",header = TRUE,sep = ",")
View(sales_table_txt)

sales_delim_txt <- read.delim("yearly_sales.csv",sep = ",")
...
## Agregar una columna para el promedio de las ventas por orden
sales$per_order <- sales$sales_total/sales$num_of_orders

## Exportar la información sin nombres de columnas
write.table(sales,"sales_modified.txt",sep = "\t",col.names = FALSE)

## Exportar la información con nombres de columnas
write.table(sales,"yearly_sales.csv",sep = "\t",col.names = TRUE)

yearly_sales_exported_data <- read.csv("yearly_sales.csv")

yearly_sales_exported_data

## Exportar con columnas modificadas
...
## Exportar las salidas a archivos
# Guardar como jpg
...
i <- 100                #creates a numeric variable
sport <- 101
